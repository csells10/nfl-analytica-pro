    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-start gap-3 rounded-md border border-border bg-background/40 p-3 text-left transition-colors hover:border-primary/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-foreground">{run.display_label}</p>
          {run.scope_label && <p className="text-xs text-muted-foreground">{run.scope_label}</p>}
      ...
      <CollapsibleContent>
        <div className="mt-2 grid grid-cols-2 gap-3 rounded-md border border-border bg-muted/40 p-3 sm:grid-cols-3">
          ...
          <Field label="Duration" value={duration(run.duration_ms)} />
      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex w-full items-start gap-3 p-4 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground">
              An attempt is one bounded coordinator invocation. It may cover one game or many, and is not the same
              as a game ID.
            </p>
          </div>
        ...
        <CollapsibleContent>
          <CardContent className="space-y-2 p-4 pt-0">
            {runs.length === 0 ? (
              <p className="text-xs text-muted-foreground">No attempts recorded in this range.</p>
            ) : (
              runs.map((run) => <RunRow key={run.attempt_id} run={run} />)
            )}
          </CardContent>
        </CollapsibleContent>
      </Collapsible>
